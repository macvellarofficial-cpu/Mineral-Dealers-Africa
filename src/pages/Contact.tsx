...
export default function Contact() {
  ...
  return (
    <div className="flex flex-col gap-10 text-left" id="contact-us-layout">
      ...
      <div className="luxury-glass border border-[#D4AF37]/20 p-8 rounded-3xl relative overflow-hidden shadow-[0_15px_35px_rgba(0,0,0,0.5),0_0_20px_rgba(212,175,55,0.03)] font-sans">
        <div className="absolute top-[-40%] right-[-10%] w-[50%] h-[150%] rounded-full bg-[#D4AF37]/3.5 blur-[120px] pointer-events-none" />
        <span className="text-[10px] uppercase font-mono tracking-[0.25em] text-[#D4AF37] block mb-2 font-extrabold">{t('contact.tag')}</span>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative z-10">
        ...
        <div className="lg:col-span-12 xl:col-span-5 flex flex-col gap-6">
          ...
          <div className="luxury-glass border border-[#D4AF37]/10 rounded-2xl p-6 shadow-md flex flex-col gap-5">
            <div className="flex items-start gap-4">
              ...
              <div>
                <span className="block text-[10px] uppercase font-mono text-white/40 tracking-wider">{t('contact.office.addressLabel')}</span>
                <p className="text-xs text-white leading-relaxed font-medium mt-0.5">
                  {t('contact.office.addressVal')}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <span className="block text-[10px] uppercase font-mono text-white/40 tracking-wider">{t('contact.office.phoneLabel')}</span>
            ...
            <div className="flex items-start gap-4">
              ...
              <div>
                <span className="block text-[10px] uppercase font-mono text-white/40 tracking-wider">{t('contact.office.emailLabel')}</span>
            ...
            <div className="flex items-start gap-4 pt-4 border-t border-white/5">
              ...
              <div>
                <span className="block text-[10px] uppercase font-mono text-white/40 tracking-wider">{t('contact.office.hoursLabel')}</span>
                <p className="text-xs text-white leading-relaxed font-medium mt-0.5 font-mono">
                  {t('contact.office.hoursVal')}<br />
                  {t('contact.office.satVal')}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-red-950/15 p-5 rounded-2xl border border-red-500/20 shadow-[0_0_15px_rgba(239,68,68,0.055)] flex gap-3 text-left">
            ...
            <div>
              <h4 className="font-serif text-xs uppercase tracking-widest font-extrabold text-[#ef4444] mb-1 font-bold">
                {t('contact.safety.title')}
              </h4>
              <p className="text-[11px] text-white/55 leading-relaxed font-light">
                {t('contact.safety.desc')}
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-12 xl:col-span-7 luxury-glass border border-[#D4AF37]/18 rounded-3xl p-6 md:p-8 shadow-[0_15px_45px_rgba(0,0,0,0.5)]">
          <h3 className="font-serif text-xs uppercase tracking-widest font-extrabold text-white mb-4">{t('contact.form.title')}</h3>

          {success ? (
            <div className="py-12 text-center flex flex-col items-center gap-4">
              <CheckCircle className="h-12 w-12 text-emerald-400 animate-pulse" />
              <h4 className="font-serif text-lg font-bold text-white">{t('contact.success.title')}</h4>
              <p className="text-xs text-white/60 max-w-xs mx-auto leading-relaxed font-light">
                {t('contact.success.desc')}
              </p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <span className="text-[10px] block uppercase font-mono text-[#D4AF37] tracking-wider mb-2 font-bold">{t('contact.form.tag')}</span>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] text-white/50 uppercase font-mono tracking-wider mb-1">{t('contact.form.name')} *</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] text-white/50 uppercase font-mono tracking-wider mb-1">{t('contact.form.org')}</label>
                  <input
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] text-white/50 uppercase font-mono tracking-wider mb-1">{t('contact.form.phone')} *</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] text-white/50 uppercase font-mono tracking-wider mb-1">{t('contact.form.email')} *</label>
                  <input
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-[10px] text-white/50 uppercase font-mono tracking-wider mb-1">{t('contact.form.category')}</label>
                  <select
                    value={mineral} onChange={(e) => setMineral(e.target.value)}
                    className="w-full bg-black/45 border border-[#D4AF37]/15 focus:border-[#D4AF37] text-white text-xs rounded-lg p-2.5 outline-none font-medium transition-all"
                  >
                    <option value="Gold Refined Bullion" className="bg-neutral-900">Gold Refined Bullion (24K)</option>
                    <option value="Gold Raw Nuggets" className="bg-neutral-900">Gold Raw Nuggets</option>
                    <option value="Rough Diamonds Collection" className="bg-neutral-900">Rough Diamonds Collection</option>
                    <option value="Emeralds / Sapphires" className="bg-neutral-900">Emeralds / Sapphires</option>
                    <option value="Strategic Battery Metals" className="bg-neutral-900">Strategic Battery Metals</option>
                  </select>
                </div>
                <div>
                  <label className="block text-[10px] text-white/50 uppercase font-mono tracking-wider mb-1">{t('contact.form.volume')}</label>
                  <input
              ...
              <div>
                <label className="block text-[10px] text-white/50 uppercase font-mono tracking-wider mb-1">{t('contact.form.message')}</label>
                <textarea
              ...
              <div className="bg-[#D4AF37]/5 p-3 rounded-lg border border-[#D4AF37]/15 text-[10px] text-white/60 leading-normal flex items-start gap-2 select-none font-light">
